...
## Seed: Create Test User (run once)

`login.php` uses PHP's `password_verify()` (bcrypt), so the hash must be generated by PHP — not SQL. Run this script once via browser or CLI:

```php
<?php
// Run at: http://localhost/Cafe/tests/seed_test_user.php
...
Delete `seed_test_user.php` after running — it is a one-time setup file.
